...
async def _start(bot, update):
  await update.reply_text(f"** Tiktok Bot Downloader - Format ( Mp3 File , Video ) **", True, reply_markup=InlineKeyboardMarkup(START_BUTTONS))
...
async def _tiktok(bot, update):
  ...
  if not 'tiktok.com' in resp.url:
    return
  await update.reply('Select The Type From Below', True, reply_markup=InlineKeyboardMarkup(DL_BUTTONS))
